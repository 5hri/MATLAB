...
% for loop to calc for all faults 
    for i=1:21                          % # of fault data sets
        ...
        eval(['avg_data',num2str(i),'(j,1)','=','0.5','*',...
            '(min(Tcrt',num2str(i),'(:,j))','+','max(Tcrt',num2str(i),'(:,j)))',';']);
...
for i=1:size(Tcrt05sd,2)
    ...
    avg_data05sd(i,1) = 0.5*(min(Tcrt05sd(:,i)) + max(Tcrt05sd(:,i)));
    std_PC05sd = [std_PC05sd; std(Tcrt05sd(:,i))];
end

for i=1:size(Tcrt1sd,2)
    mini1sd = [mini1sd; min(Tcrt1sd(:,i))];
    maxi1sd = [maxi1sd; max(Tcrt1sd(:,i))];
    avg_data1sd(i,1) = 0.5*(min(Tcrt1sd(:,i)) + max(Tcrt1sd(:,i)));
    std_PC1sd = [std_PC1sd; std(Tcrt1sd(:,i))];
end

for i=1:size(Tcrt2sd,2)
    mini2sd = [mini2sd; min(Tcrt2sd(:,i))];
    maxi2sd = [maxi2sd; max(Tcrt2sd(:,i))];
    avg_data2sd(i,1) = 0.5*(min(Tcrt2sd(:,i)) + max(Tcrt2sd(:,i)));
    std_PC2sd = [std_PC2sd; std(Tcrt2sd(:,i))];
end
...
area = []; area05sd = []; area1sd = [];area2sd = [];
    ...
    for i=1:21
        ...
        eval(['centroid',num2str(i),'(j)','=','(abs(bb)+2*abs(aa))','/','(abs(bb)+abs(aa))','/','3',';']);
        eval(['area',num2str(i),'(j)','=','(abs(bb)+abs(aa))','/','2',';']);
        end
    end    
...
for i=1:size(Tcrt,2)-1
    ...
    centroid(i) = (abs(bb) + 2*abs(aa))/(abs(bb) + abs(aa))/3;
    area(i) = (abs(bb) + abs(aa))/2;
end

for i=1:size(Tcrt05sd,2)-1
    bb = max(Tcrt05sd(:,i)) - min(Tcrt05sd(:,i));
    aa = max(Tcrt05sd(:,i + 1)) - min(Tcrt05sd(:,i + 1));
    centroid05sd(i) = (abs(bb) + 2*abs(aa))/(abs(bb) + abs(aa))/3;
    area05sd(i) = (abs(bb) + abs(aa))/2;
end
for i=1:size(Tcrt1sd,2)-1
    bb = max(Tcrt1sd(:,i)) - min(Tcrt1sd(:,i));
    aa = max(Tcrt1sd(:,i + 1)) - min(Tcrt1sd(:,i + 1));
    centroid1sd(i) = (abs(bb) + 2*abs(aa))/(abs(bb) + abs(aa))/3;
    area1sd(i) = (abs(bb) + abs(aa))/2;
end
for i=1:size(Tcrt2sd,2)-1
    bb = max(Tcrt2sd(:,i)) - min(Tcrt2sd(:,i));
    aa = max(Tcrt2sd(:,i + 1)) - min(Tcrt2sd(:,i + 1));
    centroid2sd(i) = (abs(bb) + 2*abs(aa))/(abs(bb) + abs(aa))/3;
    area2sd(i) = (abs(bb) + abs(aa))/2;
end
%% Centroid plotting 
figure()
subplot(2,1,1);
...
% faultsmatrix_exp = zeros(size(Tcrt,1),size(Tcrt,2));
% cutoff_up= CI_up(:,2)';cutoff_low=CI_low(:,2)';
% for i=1:size(Tcrt,1)
%     for j=1:size(Tcrt,2)
%         if Tcrt1(i,j)> cutoff_up(1,j) || Tcrt1(i,j) < cutoff_low(1,j)
%             faultsmatrix(i,j)=1;
%         end
%     end
% end
% for i=1:size(Tcrt,1)
%     for j=1:size(Tcrt,2)
%         if Tcrt(i,j)> cutoff_up(1,j) || Tcrt(i,j) < cutoff_low(1,j)
%             faultsmatrix_exp(i,j)=1;
%         end
%     end
% end
...
%% For Friday 7 Nov meeting Fault 3,9,15,21 Centroid plotting 
figure()
subplot(2,1,1);
aa = (1:size(Tcrt,2))-0.5;    
centroidaxis = aa(1,2:size(aa,2)); % used 2:size..to omit first x axis label
...
plot(centroidaxis,centroid,'LineStyle','-',...
   'Marker',markers(1,1),'color',color_code(1,:),'linewidth',1.15); hold on
...
plot([1:size(Tcrt,2)], maxi, 'LineStyle','-',...
   'Marker',markers(1,1),'color',color_code(1,:),'linewidth',1.15); hold on
plot([1:size(Tcrt1,2)], maxi1,'LineStyle','--',...
   'Marker',markers(1,2),'color',color_code(2,:),'linewidth',1.15);hold on
plot([1:size(Tcrt3,2)], maxi3,'LineStyle','--',...
   'Marker',markers(1,4),'color',color_code(5,:),'linewidth',1.15);hold on
plot([1:size(Tcrt9,2)], maxi9,'LineStyle','--',...
   'Marker',markers(1,10),'color',color_code(11,:),'linewidth',1.15);hold on
plot([1:size(Tcrt15,2)], maxi15,'LineStyle','--',...
   'Marker',markers(1,16),'color',color_code(17,:),'linewidth',1.15);hold on
plot([1:size(Tcrt21,2)], maxi21,'LineStyle','--',...
   'Marker',markers(1,22),'color',color_code(23,:),'linewidth',1.15);hold on
...
plot([1:size(Tcrt,2)], mini, 'LineStyle','-',...
   'Marker',markers(1,1),'color',color_code(1,:),'linewidth',1.15); hold on
plot([1:size(Tcrt1,2)], mini1,'LineStyle','--',...
   'Marker',markers(1,2),'color',color_code(2,:),'linewidth',1.15);hold on
plot([1:size(Tcrt3,2)], mini3,'LineStyle','--',...
   'Marker',markers(1,4),'color',color_code(5,:),'linewidth',1.15);hold on
plot([1:size(Tcrt9,2)], mini9,'LineStyle','--',...
   'Marker',markers(1,10),'color',color_code(11,:),'linewidth',1.15);hold on
plot([1:size(Tcrt15,2)], mini15,'LineStyle','--',...
   'Marker',markers(1,16),'color',color_code(17,:),'linewidth',1.15);hold on
plot([1:size(Tcrt21,2)], mini21,'LineStyle','--',...
   'Marker',markers(1,22),'color',color_code(23,:),'linewidth',1.15);hold on
...
plot(centroidaxis,centroid1,'LineStyle','--',...
   'Marker',markers(1,2),'color',color_code(2,:),'linewidth',1.15);hold on
plot(centroidaxis,centroid3,'LineStyle','--',...
   'Marker',markers(1,4),'color',color_code(5,:),'linewidth',1.15);hold on
plot(centroidaxis,centroid9,'LineStyle','--',...
   'Marker',markers(1,10),'color',color_code(11,:),'linewidth',1.15);hold on
plot(centroidaxis,centroid15,'LineStyle','--',...
   'Marker',markers(1,16),'color',color_code(17,:),'linewidth',1.15);hold on
plot(centroidaxis,centroid21,'LineStyle','--',...
   'Marker',markers(1,22),'color',color_code(23,:),'linewidth',1.15);hold on

xlabel('Principal Components')
ylabel('Centroid & Min/Max of Principal components')
legend ('Centroid NO','Normal Operating data','Fault # 1','Fault # 3',...
    'Fault # 9','Fault # 15','Fault # 21');
grid on
%hold off;
...
%% For Friday 7 Nov meeting Fault 3,9,15,21 Trapezoid Area plotting 

figure()
subplot(2,1,1);
aa = (1:size(Tcrt,2))-0.5;    
centroidaxis = aa(1,2:size(aa,2)); % used 2:size..to omit first x axis label
plot(centroidaxis,area,...
   'Marker',markers(1,1),'color',color_code(1,:),'linewidth',1.15); hold on
...
plot(centroidaxis,area1,...
   'Marker',markers(1,2),'color',color_code(2,:),'linewidth',1.15);hold on
plot(centroidaxis,area3,...
   'Marker',markers(1,4),'color',color_code(5,:),'linewidth',1.15);hold on
plot(centroidaxis,area9,...
   'Marker',markers(1,10),'color',color_code(11,:),'linewidth',1.15);hold on
plot(centroidaxis,area15,...
   'Marker',markers(1,16),'color',color_code(17,:),'linewidth',1.15);hold on
plot(centroidaxis,area21,...
   'Marker',markers(1,22),'color',color_code(23,:),'linewidth',1.15);hold on
xlabel('Principal Components')
ylabel('Area & Min/Max of Principal components')
legend ('Area NO','Normal Operating data','Fault # 1','Fault # 3',...
    'Fault # 9','Fault # 15','Fault # 21');
grid on
...
grid on
hold off;
%% modified data (161 to 960 samples) for fault 3 Vs all data fault 3
figure()
plot(centroidaxis,centroid_modified3,'r*');hold on
plot(centroidaxis,centroid3,'g*');hold on
axis([1 10 0.47 0.52])
figure()
plot(centroidaxis,area_modified3,'r*');hold on
plot(centroidaxis,area3,'g*');hold on
