...
% for i=1:n
%     T2(i)=Tcrt3(i,:)*inv(diag(Eref(1:a)))*Tcrt3(i,:)';
%     Q(i)=Xcrt3(i,:)*(eye(p)-Pref(:,1:a)*Pref(:,1:a)')*Xcrt3(i,:)';
% end
...
for i=1:size(Tcrt05sd,2)
    ...
    avg_data05sd(i,1) =  mean(Tcrt05sd(:,i));
...
for i=1:size(Tcrt1sd,2)
    ...
    avg_data1sd(i,1) =  mean(Tcrt1sd(:,i));
...
for i=1:size(Tcrt2sd,2)
    ...
    avg_data2sd(i,1) =  mean(Tcrt2sd(:,i));
...
figure()
plot(Tcrt1(1:160,1),'r');hold on
plot(Tcrt1(161:960,1),'b');hold on
